    ...
    public class AdminPedidosController : Controller
    {
        ...
        public async Task<IActionResult> Index(string filter, int pageindex = 1, string sort = "Nome")
        {
            var resultado = _context.Pedidos.AsNoTracking()
              .AsQueryable();//Converte a coleção em um tipo IQueryble
            if (!string.IsNullOrWhiteSpace(filter))
            {
                resultado = resultado.Where(p => p.Nome.Contains(filter));
            }
            var model = await PagingList.CreateAsync(resultado, 5, pageindex, sort, "Nome"); //Criando dados paginados (Consulta, tamanho da pagina, pageindex, criterio de ordenação )
